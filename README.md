...
- 同一套数据支持 Classic、Swiss Poster、Midnight Buddy 与 Cosmic Buddy 四种展示页面，可独立编译和快速烧录切换。
...
```bash
pio run -e deskburn_cosmic
pio run -e deskburn_cosmic -t upload
```

四版使用相同的 BLE 协议和数据缓存，Mac 采集端无需修改。展示效果、可直接烧录的
...
firmware/
  ...
  deskburn_cosmic/         Cosmic Buddy 月夜精灵页面及专用资源
  releases/                可直接烧录的四套展示固件
...
tools/
  ...
  generate_cosmic_assets.py Cosmic Buddy 背景、字体与预览生成器
...
- [屏幕展示版本](docs/展示版本.md)：四套页面效果、切换和固件成品。
