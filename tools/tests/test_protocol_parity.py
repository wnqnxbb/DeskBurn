...
int main(int argc, char** argv) {
  ...
  printf("{\"accepted\": true, \"today_milli\": %u, \"today_tokens\": %u,"
         " \"week_milli\": %u, \"month_milli\": %u, \"total_milli\": %u,"
         " \"epoch\": %u, \"version\": %u,"
         " \"struct_size\": %u, \"crc_check\": %u}\n",
         packet.todayMilliUsd, packet.todayTokens, packet.weekMilliUsd,
         packet.monthMilliUsd, packet.totalMilliUsd, packet.epoch,
         packet.version,
...
class ProtocolParityTest(unittest.TestCase):
    ...
    def test_firmware_accepts_python_packet(self) -> None:
        ...
        snapshot = UsageSnapshot(
            ...
            week_cost_usd=137.68, month_cost_usd=1651.86,
            total_cost_usd=3006.39, updated_at=1_785_147_325,
        )
        ...
        self.assertEqual(decoded["today_tokens"], 105_898_081)
    ...
    def test_firmware_rejects_v1_packet(self) -> None:
        """v1 的 26 字节包必须被拒收。

        加字段时升了版本号,就是为了让旧包在新固件上整包丢弃。万一漏升,v1 包
        会因长度不符被拒,但同长度的畸形包就可能被按新布局错位读出来。
        """
        snapshot = protocol.fake_snapshot()
        body = struct.pack(
            "<HBBIIIII", protocol.MAGIC, 1, protocol.FLAG_NONE,
            137_680, 105_898_081, 137_680, 1_651_860, snapshot.updated_at,
        )
        v1_packet = body + struct.pack("<H", protocol.crc16_ccitt(body))
        self.assertEqual(len(v1_packet), 26)

        self.assertFalse(self._decode_in_cpp(v1_packet)["accepted"])
